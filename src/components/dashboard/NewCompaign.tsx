...
export function NewComapaignDialog({ children }: { children: ReactNode }) {
  ...
  const handleFormSubmit = async (data: FieldValues) => {
    try {
      ...
      const response = await postReq({
        data: requestData,
        url: "/api/compaign/create",
      });

      if (response?.ok || response?.status === 201) {
        toast({
          title: "Compaign created successfully",
        });
        ...
        // start bot scraper
        try {
          const { compaign } = await response.json();
          const botResponsePromise = botPostReq({
            data: {
              domains: compaign?.domains?.map(
                (data: { domain: string; status: string; scrapedAt: string }) =>
                  data?.domain
              ),
              uid: user?.uid || "",
